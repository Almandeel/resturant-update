						...
						<tfoot>
							{{--  <tr>
								...
								<td>
									...
									<input type="number" min="0" class="form-control input input-quantity" name="quantities[]" value="0" />
								</td>
								<td>
									<div class="input-group">
										<label class="input-group-addon">
								...
								<td>
									<button type="button" class="btn btn-primary btn-xs tfoot-btn-add-row"><i class="fa fa-plus"></i></button>
								</td>
							</tr>  --}}
							<tr>
								<th></th>
		...
		@if (!$bill)
			...
			$(function () {
				...
				$(document).bind('keyup change', 'input.input-payment', function(e) {
					...
					/*let safe_id = input.data('safe');
					let safe = input.closest('table').find('.payments-select-safes option[value='+safe_id+']')
